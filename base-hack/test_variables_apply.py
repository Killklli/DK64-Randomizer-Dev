...
_V='- Writing '
_U='homing_balloons'
_T='hud_bp_multibunch'
_S='rambi_enguarde_pickup'
_R='caves_kosha_dead'
_Q='textbox_hold'
_P='vanilla_fixes'
_O='galleon_star'
...
_A=True
import os
set_variables={'level_order_rando_on':0,'troff_scoff_count':[25,200,300,400,410,420,8],'blocker_normal_count':[2,3,4,5,6,7,8,9],'unlock_kongs':31,'unlock_moves':1,'fast_start_beginning':1,'camera_unlocked':1,'tag_anywhere':1,'fast_start_helm':1,'crown_door_open':0,'coin_door_open':0,_B:{_F:_A,_G:_A,_H:_A,_I:_A,_J:_A,_K:_A,_L:_A,_M:_A,_N:_A,_O:_A,_P:_A,_Q:_A,_R:_A,_S:_A,_T:_A,_U:_A},'price_rando_on':1,'k_rool_order':[2,0,4,3,1],'damage_multiplier':0,'fps_on':1,'no_health_refill':0,'slam_prices':[4,5],'gun_prices':[1,2,3,4,5],'instrument_prices':[1,2,3,4,5],'gun_upgrade_prices':[1,2],'ammo_belt_prices':[1,2],'instrument_upgrade_prices':[1,2,3],'move_rando_on':1,'remove_blockers':127,'resolve_bonus':0,'disable_drops':0,'shop_indicator_on':2,'warp_to_isles_enabled':1,'lobbies_open_bitfield':255,'perma_lose_kongs':0,'jetpac_medal_requirement':1,'free_target_llama':0,'free_source_llama':3,'keys_preturned':0,'short_bosses':1,'fast_warp':1,'activate_all_bananaports':1,'piano_game_order':[5,0,1,3,5,5,3],'dartboard_order':[0,1,2,3,4,5],'fast_gbs':1,'remove_high_requirements':1,'open_level_sections':1,'auto_keys':0,_E:[204,0],'klaptrap_color_bbother':150,'kut_out_phases':[3,2,0],'dpad_visual_enabled':1,'helm_order':[2,0,255,255,255],'disco_chunky':1,'krusha_slot':2,'starting_kong':2,'kut_out_kong_order':[1,0,0,0,0],'helm_hurry_mode':0,'win_condition':5,'version':2,'item_rando':1,'vulture_item':72,'japes_rock_item':76,'medal_cb_req':5,'hard_enemies':1,'remove_oscillation_effects':1,'starting_map':26,'starting_exit':1,'rareware_gb_fairies':1}
...
def writeToROMNoOffset(offset,value,size,name):
	'Write to ROM without offset.';B=value;A=offset;print(_V+name+_W+hex(A)+') to '+str(B))
	with open(_C,_D)as C:C.seek(A);C.write(bytearray(valtolst(B,size)))
def writeToROM(offset,value,size,name):
	'Write byte data to rom.';B=value;A=offset;print(_V+name+_W+hex(A)+') to '+str(B))
	with open(_C,_D)as C:C.seek(33476640+A);C.write(bytearray(valtolst(B,size)))
with open('include/variable_space_structs.h','r')as varspace:
	...
	for x in test_keys:
		if x==_E:
			...
			order=[_F,_G,_H,_I,_J,_K,_L,_M,_N,_O,_P,_Q,_R,_S,_T,_U]
