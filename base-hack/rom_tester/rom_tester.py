...
def getMove(fh,offset,kong,shop,level):
	...
	if C==65535:
		A=B>>24;D=A>>5&7;E=(A>>3&3)+1;F=A&7
		if D==7:return'No Upgrade'
		return f"{move_types[D]} level {str(E)} (Kong {F})"
	else:return f"Flag {hex(C)}"
...
if'.z64'in f:
	...
	with open(f,'rb')as fh:
		...
		for shop in range(3):
			for kong in range(5):
				for level in range(8):output(f"\t\t{kongs[kong]} {shops[shop]} {levels_isles[level]}: {getMove(fh,level+8*kong+40*shop,kongs[kong],shops[shop],levels_isles[level])}")
		for tbarrel in range(4):output(f"\t\tTraining Barrel {tbarrel}: {getMove(fh,120+tbarrel,0,0,0)}")
		output(f"\t\tBFI: {getMove(fh,124,0,0,0)}");output(f"\tKut Out Kong Order:")
