...
def writeVanillaMoveData(fh):
	...
	for B in range(5):
		for C in range(7):A.write(bytearray([getWrite(cranky_arr[B][C],B)]))
	for B in range(5):
		for C in range(7):A.write(bytearray([getWrite(funky[C],B)]))
	for B in range(5):
		for C in range(7):A.write(bytearray([getWrite(candy[C],B)]))