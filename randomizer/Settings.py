...
class Settings:
	...
	def resolve_settings(A):
		'Resolve settings which are not directly set through the UI.';M='random';L='random_helm';K='levels';H='all';G='vanilla';C=GetKongs()
		if A.random_prices!=G:A.prices=RandomizePrices(A.random_prices)
		if A.kong_rando:
			A.starting_kong=random.choice(C)
			if A.shuffle_loading_zones==K:A.kongs_for_progression=_C
			elif A.shuffle_loading_zones==_D:A.diddy_freeing_kong=A.starting_kong
			else:A.diddy_freeing_kong=random.choice(C)
			A.lanky_freeing_kong=random.choice(C);A.tiny_freeing_kong=random.choice([Kongs.diddy,Kongs.chunky]);A.chunky_freeing_kong=random.choice(C)
		...
		B.append(Kongs.chunky);A.krool_order=B;I=[Events.JapesKeyTurnedIn,Events.AztecKeyTurnedIn,Events.FactoryKeyTurnedIn,Events.GalleonKeyTurnedIn,Events.ForestKeyTurnedIn,Events.CavesKeyTurnedIn,Events.CastleKeyTurnedIn,Events.HelmKeyTurnedIn];F=I.copy();J=A.krool_key_count
		if A.krool_access==L:A.krool_keys_required.append(Events.HelmKeyTurnedIn);F.remove(Events.HelmKeyTurnedIn);J-=1
		if A.krool_access==G:A.krool_keys_required.extend([Events.FactoryKeyTurnedIn,Events.HelmKeyTurnedIn])
		elif A.krool_access==H:A.krool_keys_required.extend(I)
		elif A.krool_access==M or A.krool_access==L:
			random.shuffle(F)
			for N in range(J):A.krool_keys_required.append(F[N])
		...
		if A.bonus_barrel_rando:A.bonus_barrels=M
		if A.level_randomization=='level_order':A.shuffle_loading_zones=K
		elif A.level_randomization=='loadingzone':A.shuffle_loading_zones=H
		elif A.level_randomization=='loadingzonesdecoupled':A.shuffle_loading_zones=H;A.decoupled_loading_zones=_C
		elif A.level_randomization==G:A.shuffle_loading_zones=_D
