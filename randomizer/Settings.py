...
class Settings:
	...
	def __init__(A,form_data):
		...
		A.seed_id=str(A.seed);A.seed=str(A.seed)+A.__hash;A.set_seed();A.seed_hash=[random.randint(0,9)for A in range(5)];A.krool_keys_required=[];A.blocker_max=A.blocker_text if A.blocker_text else 50;A.troff_max=A.troff_text if A.troff_text else 270;A.troff_min=[0.25,0.3,0.35,0.4,0.45,0.5,0.55];A.training_barrels='startwith';A.shuffle_items=_D;A.unlock_all_moves=_B;A.progressive_upgrades=_B;A.prices=VanillaPrices.copy();A.level_order={1:Levels.JungleJapes,2:Levels.AngryAztec,3:Levels.FranticFactory,4:Levels.GloomyGalleon,5:Levels.FungiForest,6:Levels.CrystalCaves,7:Levels.CreepyCastle};A.resolve_settings()
	...
	def generate_main(A):'Set Default items on main page.';A.seed=_A;A.download_patch_file=_A;A.bonus_barrel_rando=_A;A.loading_zone_coupled=_A;A.move_rando=_A;A.random_patches=_A;A.random_prices=_A;A.boss_location_rando=_A;A.boss_kong_rando=_A;A.kasplat_rando_setting=_A;A.puzzle_rando=_A;A.shuffle_shops=_A
	...
	def SelectKongLocations(B):
		...
		C=B.open_levels or B.activate_all_bananaports==_E
