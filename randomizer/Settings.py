...
class Settings:
	...
	def resolve_settings(A):
		...
		B.append(Kongs.chunky);A.krool_order=B;L=[Events.JapesKeyTurnedIn,Events.AztecKeyTurnedIn,Events.FactoryKeyTurnedIn,Events.GalleonKeyTurnedIn,Events.ForestKeyTurnedIn,Events.CavesKeyTurnedIn,Events.CastleKeyTurnedIn,Events.HelmKeyTurnedIn];E=L.copy();G=A.krool_key_count
		if A.krool_access:A.krool_keys_required.append(Events.HelmKeyTurnedIn);E.remove(Events.HelmKeyTurnedIn);G-=1
		random.shuffle(E)
		for M in range(G):A.krool_keys_required.append(E[M])
