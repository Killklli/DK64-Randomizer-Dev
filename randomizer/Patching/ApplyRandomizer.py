...
def patching_response(responded_data):
	'Response data from the background task.\n\n    Args:\n        responded_data (str): Pickled data (or json)\n    ';l='spoiler_log_text';k='tracker_text';j='spoiler_log_block';i='shift';h='big';g='error';S=responded_data;R='value';Q='base64';G='';E=asyncio.get_event_loop()
	try:
		T=json.loads(S)
	...
	if A.settings.shuffle_loading_zones=='levels':
		...
		if Types.Key not in A.settings.shuffled_location_types:
			for (D,V) in p.items():q=A.shuffled_exit_data.get(D).reverse;ROM().seek(B+30+C);ROM().writeMultipleBytes(J[int(q)],2);C+=2
		else:
	...
	if A.settings.starting_kongs_count==5:ROM().seek(B+44);ROM().write(31)
		...
		W=0
		for r in A.settings.starting_kong_list:W|=1<<r
	...
	if A.settings.free_trade_items:ROM().seek(B+275);K=int.from_bytes(ROM().readBytes(1),h);ROM().seek(B+275);ROM().write(K|1)
	if A.settings.free_trade_blueprints:ROM().seek(B+275);K=int.from_bytes(ROM().readBytes(1),h);ROM().seek(B+275);ROM().write(K|2)
	if A.settings.quality_of_life:
		...
		for H in QoLSelector:
			if H[R]in L:s=int(H[i]>>3);t=int(H[i]%8);X[s]|=128>>t
		ROM().seek(B+176)
		for u in X:ROM().writeMultipleBytes(u,1)
	ROM().seek(B+165)
	...
	if A.settings.win_condition in Z:v=Z.index(A.settings.win_condition);ROM().seek(B+285);ROM().write(v)
	M=[0,1,2,3,4,5,6,7]
	...
	if len(A.settings.enemies_selected)==0 and(A.settings.enemy_rando or A.settings.crown_enemy_rando!='off'):
		b=[]
		for w in EnemySelector:b.append(w[R])
	...
	if A.settings.generate_spoilerlog is True:js.document.getElementById(j).style.display=G;E.run_until_complete(GenerateSpoiler(A.json));js.document.getElementById(k).value=generateTracker(A.json)
	else:js.document.getElementById(l).innerHTML=G;js.document.getElementById(l).value=G;js.document.getElementById(k).value=G;js.document.getElementById(j).style.display='none'
	...
	for (d,V) in c.items():
		...
		if d not in e:
			...
			f=I[O].insertRow(-1);y=f.insertCell(0);z=f.insertCell(1);y.innerHTML=d;z.innerHTML=FormatSpoiler(V)
