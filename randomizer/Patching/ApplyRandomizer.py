...
def patching_response(responded_data):
	'Response data from the background task.\n\n    Args:\n        responded_data (str): Pickled data (or json)\n    ';U='settings_table';T='none';S='spoiler_log_text';R='spoiler_log_block';Q='nav-settings-tab';P='seed';O='base64';N='error';H=responded_data
	try:
		I=json.loads(H)
		if I.get(N):V=I.get(N);ProgressBar().set_class('bg-danger');ProgressBar().update_progress(10,f"Error: {V}");ProgressBar().reset();return None
	except Exception:pass
	ProgressBar().update_progress(8,'Applying Patches');A=pickle.loads(codecs.decode(H.encode(),O));A.settings.verify_hash();Settings({P:0}).compare_hash(A.settings.public_hash);A.settings.set_seed()
	if A.settings.download_patch_file:A.settings.download_patch_file=False;js.save_text_as_file(codecs.encode(pickle.dumps(A),O).decode(),f"dk64-{A.settings.seed_id}.lanky")
	B=33476640
	if A.settings.shuffle_loading_zones=='levels':
		...
		for (E,J) in Z.items():b=A.shuffled_exit_data.get(E).reverse;ROM().seek(B+30+C);ROM().writeMultipleBytes(a[int(b)],2);C+=2
	C=0
	...
	if A.settings.fast_start_hideout_helm:ROM().seek(B+49);ROM().write(1)
	...
	if A.settings.generate_spoilerlog is True:js.document.getElementById(Q).style.display='';js.document.getElementById(R).style.display='';js.document.getElementById(S).value=A.toJson()
	else:js.document.getElementById(Q).style.display=T;js.document.getElementById(S).value='';js.document.getElementById(R).style.display=T
	js.document.getElementById('generated_seed_id').innerHTML=A.settings.seed_id;d=json.loads(A.toJson())['Settings'];js.document.getElementById(U).innerHTML='';e=js.document.getElementById(U)
	for (L,J) in d.items():
		if L not in[P,'krool_phases','blocker_golden_bananas','troff_n_scoff_bananas','krool_keys_required']:M=e.insertRow(-1);f=M.insertCell(0);g=M.insertCell(1);f.innerHTML=L;g.innerHTML=J
