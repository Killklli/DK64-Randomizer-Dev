...
def patching_response(responded_data):
	'Response data from the background task.\n\n    Args:\n        responded_data (str): Pickled data (or json)\n    ';n='spoiler_log_text';m='tracker_text';l='spoiler_log_block';k='shift';j='big';i='need_rw';h='need_both';g='error';S=responded_data;R='value';Q='base64';G='';E=asyncio.get_event_loop()
	try:
		T=json.loads(S)
		if T.get(g):U=T.get(g);ProgressBar().set_class('bg-danger');js.toast_alert(U);E.run_until_complete(ProgressBar().update_progress(10,f"Error: {U}"));E.run_until_complete(ProgressBar().reset());return None
	...
	if A.settings.starting_kongs_count==5:ROM().seek(B+44);ROM().write(31)
		...
		W=0
		for t in A.settings.starting_kong_list:W|=1<<t
	...
	if A.settings.coin_door_open==h:ROM().seek(B+51);ROM().write(0)
	...
	elif A.settings.coin_door_open==i:ROM().seek(B+51);ROM().write(3)
	if A.settings.free_trade_items:ROM().seek(B+275);K=int.from_bytes(ROM().readBytes(1),j);ROM().seek(B+275);ROM().write(K|1)
	if A.settings.free_trade_blueprints:ROM().seek(B+275);K=int.from_bytes(ROM().readBytes(1),j);ROM().seek(B+275);ROM().write(K|2)
	if A.settings.quality_of_life:
		...
		for H in QoLSelector:
			if H[R]in L:u=int(H[k]>>3);v=int(H[k]%8);X[u]|=128>>v
		ROM().seek(B+176)
		for w in X:ROM().writeMultipleBytes(w,1)
	ROM().seek(B+165)
	...
	Z=['beat_krool','get_key8','all_fairies','all_blueprints','all_medals','poke_snap','all_keys']
	if A.settings.win_condition in Z:x=Z.index(A.settings.win_condition);ROM().seek(B+285);ROM().write(x)
	M=[0,1,2,3,4,5,6,7]
	...
	if A.settings.coin_door_open in[h,i]or'coin'in A.settings.item_rando_list_selected:ROM().seek(B+336);ROM().write(A.settings.medal_requirement)
	...
	if len(A.settings.enemies_selected)==0 and(A.settings.enemy_rando or A.settings.crown_enemy_rando!='off'):
		b=[]
		for y in EnemySelector:b.append(y[R])
		A.settings.enemies_selected=b
	...
	C=0;z=get_hash_images()
	for F in A.settings.seed_hash:ROM().seek(B+297+C);ROM().write(F);js.document.getElementById('hash'+str(C)).src='data:image/jpeg;base64,'+z[F];C+=1
	...
	if A.settings.generate_spoilerlog is True:js.document.getElementById(l).style.display=G;E.run_until_complete(GenerateSpoiler(A.json));js.document.getElementById(m).value=generateTracker(A.json)
	else:js.document.getElementById(n).innerHTML=G;js.document.getElementById(n).value=G;js.document.getElementById(m).value=G;js.document.getElementById(l).style.display='none'
	js.document.getElementById('generated_seed_id').innerHTML=A.settings.seed_id;c=json.loads(A.json)['Settings'];I={};O=0
	...
	for (d,V) in c.items():
		e=['Seed','algorithm']
		if d not in e:
			if I[O].rows.length>math.ceil((len(c.items())-len(e))/len(I)):O+=1
			f=I[O].insertRow(-1);A0=f.insertCell(0);A1=f.insertCell(1);A0.innerHTML=d;A1.innerHTML=FormatSpoiler(V)
