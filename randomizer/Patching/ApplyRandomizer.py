...
def patching_response(responded_data):
	'Response data from the background task.\n\n    Args:\n        responded_data (str): Pickled data (or json)\n    ';Z='settings_table';Y='tracker_text';X='spoiler_log_text';W='spoiler_log_block';V='need_rw';U='need_both';T='seed';S='base64';R='error';J=responded_data;G='';E=asyncio.get_event_loop()
	try:
		K=json.loads(J)
	...
	if A.settings.shuffle_loading_zones=='levels':
		ROM().seek(B+0);ROM().write(1);a=[Transitions.IslesMainToJapesLobby,Transitions.IslesMainToAztecLobby,Transitions.IslesMainToFactoryLobby,Transitions.IslesMainToGalleonLobby,Transitions.IslesMainToForestLobby,Transitions.IslesMainToCavesLobby,Transitions.IslesMainToCastleLobby];b=[Transitions.IslesJapesLobbyToMain,Transitions.IslesAztecLobbyToMain,Transitions.IslesFactoryLobbyToMain,Transitions.IslesGalleonLobbyToMain,Transitions.IslesForestLobbyToMain,Transitions.IslesCavesLobbyToMain,Transitions.IslesCastleLobbyToMain];C=0
		for c in a:ROM().seek(B+1+C);ROM().write(b.index(A.shuffled_exit_data[int(c)].reverse));C+=1
		d={Transitions.IslesMainToJapesLobby:Transitions.IslesJapesLobbyToMain,Transitions.IslesMainToAztecLobby:Transitions.IslesAztecLobbyToMain,Transitions.IslesMainToFactoryLobby:Transitions.IslesFactoryLobbyToMain,Transitions.IslesMainToGalleonLobby:Transitions.IslesGalleonLobbyToMain,Transitions.IslesMainToForestLobby:Transitions.IslesForestLobbyToMain,Transitions.IslesMainToCavesLobby:Transitions.IslesCavesLobbyToMain,Transitions.IslesMainToCastleLobby:Transitions.IslesCastleLobbyToMain};e={Transitions.IslesJapesLobbyToMain:26,Transitions.IslesAztecLobbyToMain:74,Transitions.IslesFactoryLobbyToMain:138,Transitions.IslesGalleonLobbyToMain:168,Transitions.IslesForestLobbyToMain:236,Transitions.IslesCavesLobbyToMain:292,Transitions.IslesCastleLobbyToMain:317};C=0
		for (F,M) in d.items():f=A.shuffled_exit_data.get(F).reverse;ROM().seek(B+30+C);ROM().writeMultipleBytes(e[int(f)],2);C+=2
	C=0
	...
	if A.settings.starting_kongs_count==5:ROM().seek(B+44);ROM().write(31)
		...
		N=0
		for g in A.settings.starting_kong_list:N|=1<<g
	...
	if A.settings.coin_door_open==U:ROM().seek(B+51);ROM().write(0)
	...
	elif A.settings.coin_door_open==V:ROM().seek(B+51);ROM().write(3)
	...
	if A.settings.coin_door_open in[U,V]:ROM().seek(B+320);ROM().write(A.settings.medal_requirement)
	...
	C=0;h=get_hash_images()
	for D in A.settings.seed_hash:ROM().seek(B+282+C);ROM().write(D);js.document.getElementById('hash'+str(C)).src='data:image/jpeg;base64,'+h[D];C+=1
	...
	if A.settings.generate_spoilerlog is True:js.document.getElementById(W).style.display=G;js.document.getElementById(X).value=A.toJson();js.document.getElementById(Y).value=generateTracker(A.toJson())
	else:js.document.getElementById(X).value=G;js.document.getElementById(Y).value=G;js.document.getElementById(W).style.display='none'
	js.document.getElementById('generated_seed_id').innerHTML=A.settings.seed_id;i=json.loads(A.toJson())['Settings'];js.document.getElementById(Z).innerHTML=G;j=js.document.getElementById(Z)
	for (P,M) in i.items():
		k=[T,'algorithm','starting_kong','starting_kong_list','diddy_freeing_kong','tiny_freeing_kong','lanky_freeing_kong','chunky_freeing_kong','medal_requirement','krool_phases','krool_keys_required','blocker_golden_bananas','troff_n_scoff_bananas','colors']
		if P not in k:Q=j.insertRow(-1);l=Q.insertCell(0);m=Q.insertCell(1);l.innerHTML=P;m.innerHTML=M
