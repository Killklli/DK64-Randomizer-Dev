...
def randomize_setup(spoiler):
	'Randomize setup.';A6='coords';A5='corner';w='offset';v='center';u='item_list';t='map';g='rotation';f='numbers';e='subtype';S='offsets';R='item';O='weight';N='type';K='positions';D='big';C=spoiler;B='rot';A='number';A7=[{R:'orange',N:86,O:3},{R:'film',N:152,O:1},{R:'crystals',N:142,O:4},{R:'standard_crate',N:143,O:4},{R:'homing_crate',N:17,O:2}];h=[]
	for x in A7:
	...
	if i:
		for F in range(216):
			...
			if F==Maps.FranticFactory:V={A5:{S:[],K:[]},'edge':{S:[],K:[]},v:{S:[],K:[]}}
			for AC in range(T):
				...
				if G==406 and C.settings.skip_arcader1 and F==Maps.FactoryBaboonBlast:ROM().seek(E+40);ROM().writeMultipleBytes(116,2);ROM().seek(E+12);ROM().writeMultipleBytes(1056964608,4)
					...
					for A1 in AB:
						for p in A1[f]:
							if p[A]==G-243:Y=A1[e];ROM().seek(E);Q=int.from_bytes(ROM().readBytes(4),D);k=int.from_bytes(ROM().readBytes(4),D);l=int.from_bytes(ROM().readBytes(4),D);V[Y][S].append({w:E,g:p[B],A:G-243});V[Y][K].append({A6:[Q,k,l],g:p[B]})
			if C.settings.puzzle_rando:
				...
				if F==Maps.FranticFactory:
					...
					for I in V:
						Y=I;I=V[I];random.shuffle(I[K])
						for (Z,L) in enumerate(I[S]):
							ROM().seek(L[w]);AJ=L[g]
							for a in range(3):
								A2=I[K][Z][A6][a]
								if a==1:A2=int(float_to_hex(1002),16)
								ROM().writeMultipleBytes(A2,4)
							q=I[K][Z][g];r=(AJ-q+4)%4;print(f"Number {L[A]} - Rotation Diff: {r}")
							if Y==v:r=random.randint(0,3)
							ROM().seek(L[w]+28);q=(2+r)%4;ROM().writeMultipleBytes(int(AI[q],16),4)
			...
			if C.settings.random_patches:
				...
				for AN in C.dirt_patch_placement:
					for J in DirtPatchLocations:
						if J.map_id==F and J.name==AN:
							if M in c:
								while M in c:M+=1
							print(J.name);H=[];H.append(int(float_to_hex(J.x),16));H.append(int(float_to_hex(J.y),16));H.append(int(float_to_hex(J.z),16));H.append(int(float_to_hex(1),16))
							for Q in range(8):H.append(0)
							AO=hex(J.rotation)+'007B';H.append(int(AO,16));AP=hex(M)+'46D0';c.append(M);M+=1;H.append(int(AP,16));b.append(H)
