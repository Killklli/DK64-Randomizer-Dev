...
def place_randomized_items(spoiler):
	'Place randomized items into ROM.';Y='shared';X='kong';W='id';S='upscale';P=spoiler;O='flag';N='big';M=None;G='obj'
	if P.settings.shuffle_items:
			...
			if A.can_have_item:
				if A.is_shop:
					...
					for Z in A.placement_index:
						R=f+4*Z
						if A.new_item is M:
							ROM().seek(R);g=int.from_bytes(ROM().readBytes(4),N)
							...
							ROM().seek(R);ROM().writeMultipleBytes(J<<5,1);ROM().writeMultipleBytes(A.price,1);ROM().writeMultipleBytes(A.new_flag,2)
				elif not A.reward_spot:
					for E in A.placement_data:
						if E not in I:I[E]=[]
						if A.new_item is M:I[E].append({W:A.placement_data[E],G:Types.NoItem,X:0,O:0,S:1,Y:False})
					...
					if A.old_item==Types.Blueprint:U=A.old_flag-469;ROM().seek(33492992+U);ROM().write(B)
						...
						n=[Types.Banana,Types.Blueprint,Types.Key,Types.Crown,Types.Coin,Types.Medal,Types.Shop,Types.Shop,Types.Shop,Types.TrainingBarrel,Types.Shockwave,M];U=A.old_flag-549;ROM().seek(33493120+U)
						if A.new_item==Types.Shop:
							K=6
							if A.new_flag in(656,657):K=6
							elif A.new_flag in(658,659):K=7
							elif A.new_flag in(660,661,662):K=8
							else:
								J=A.new_flag>>8&15
								if J==4:K=8
								elif J==2 or J==3:K=7
		...
		for E in I:
			c=js.pointer_addresses[9]['entries'][E]['pointing_to'];ROM().seek(c);q=int.from_bytes(ROM().readBytes(4),N)
			for A in range(q):
				F=c+4+A*48;ROM().seek(F+42);r=int.from_bytes(ROM().readBytes(2),N)
				for D in I[E]:
					if D[W]==r:
						ROM().seek(F+40);s=int.from_bytes(ROM().readBytes(2),N)
						if s in e:
							ROM().seek(F+40);L=0
							if D[G]==Types.Blueprint:L=model_two_indexes[Types.Blueprint][D[X]]
							elif D[G]==Types.Coin:
								L=model_two_indexes[Types.Coin][0]
								if D[O]==379:L=model_two_indexes[Types.Coin][1]
							elif D[G]==Types.Shop:
								if D[O]&32768==0:C=5
								else:
									C=D[O]>>12&7
									if D[Y]or C>5:C=5
								L=model_two_indexes[Types.Shop][C]
							else:L=model_two_indexes[D[G]]
							ROM().writeMultipleBytes(L,2);ROM().seek(F+12);t=intf_to_float(int.from_bytes(ROM().readBytes(4),N));u=t*D[S];ROM().seek(F+12);ROM().writeMultipleBytes(int(float_to_hex(u),16),4)
							if D[G]==Types.Blueprint:ROM().seek(F+4);v=intf_to_float(int.from_bytes(ROM().readBytes(4),N));w=v+D[S]*1.25;ROM().seek(F+4);ROM().writeMultipleBytes(int(float_to_hex(w),16),4)