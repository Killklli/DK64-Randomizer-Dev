...
def apply_cosmetic_colors(spoiler):
	'Apply cosmetic skins to kongs.';h='checkered';g='overalls';f='enguarde_colors';e='rambi_colors';d='chunky_colors';c='tiny_colors';b='lanky_colors';a='diddy_colors';Z='dk_colors';W='02x';V='base';T='colors';S='#000000';Q=spoiler;P='zones';L='block';J='kong_index';I='custom_setting';H='palettes';G='randomized';F='name';E='base_setting';C='image';B='kong';A='fill_type'
	if Q.settings.klaptrap_model:
		M=0;N=Q.settings.klaptrap_model
		if N=='green':M=33
		elif N=='purple':M=34
		elif N=='red':M=35
		elif N=='random_klap':M=random.randint(33,35)
		elif N=='random_model':i=[25,30,32,33,34,35,36,38,39,48,52,66,71,75,81,98,105,112,114,150,176,177,189];M=random.choice(i)
		ROM().seek(Q.settings.rom_data+310);ROM().writeMultipleBytes(M,1)
	U=[];X={}
	...
	for K in j:
		...
		for R in K[H]:
			Y=[S]
			if R[A]==h:Y=[S,S]
			O[P].append({'zone':R[F],C:R[C],A:R[A],T:Y})
		if js.document.getElementById(K[E]).value!='vanilla':
			if js.document.getElementById(K[E]).value==G:D=f"#{format(randint(0,16777215),'06x')}"
				...
				if not D:D=S
			O[P][0][T][0]=D
			if K[J]==4:O[P][1][T][0]=D;k=int(f"0x{D[1:3]}",16);l=int(f"0x{D[3:5]}",16);m=int(f"0x{D[5:7]}",16);n=f"#{format(255-k,W)}{format(255-l,W)}{format(255-m,W)}";O[P][0][T][1]=n
