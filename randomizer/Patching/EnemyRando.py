		...
		for G in range(216):
			...
			if I.settings.enemy_rando and G in g:
				for J in V:
					...
					for A in P:
						if A[O]in q:
							if G!=Maps.FranticFactory or A[X]<35 or A[X]>44:
								C=Q[c];c+=1
								if C!=Enemies.Book or G not in(Maps.CavesDonkeyCabin,Maps.JapesLankyCave,Maps.AngryAztecLobby):
									if C!=Enemies.Bug or G!=Maps.CavesDiddyLowerCabin:
