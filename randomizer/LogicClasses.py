...
class LocationLogic:
	...
	def __init__(A,id,logic,bonusBarrel=_B):'Initialize with given parameters.';A.id=id;A.logic=logic;A.bonusBarrel=bonusBarrel
class Event:
...
class Region:
	...
	def __init__(A,name,level,tagbarrel,deathwarp,locations,events,transitionFronts,restart=_B):
		'Initialize with given parameters.';B=deathwarp;A.name=name;A.level=level;A.tagbarrel=tagbarrel;A.deathwarp=_B;A.locations=locations;A.events=events;A.exits=transitionFronts;A.restart=restart;A.dayAccess=_A;A.nightAccess=_A
		if B is not _B:
			if isinstance(B,TransitionFront):A.deathwarp=B
			else:
				if B==-1:B=A.GetDefaultDeathwarp()
				A.deathwarp=TransitionFront(B,lambda l:_C)
		A.ResetAccess()
	def UpdateAccess(A,kong,logicVariables):
		...
		elif C==Kongs.donkey:A.donkeyAccess=_C
		elif C==Kongs.diddy:A.diddyAccess=_C
		elif C==Kongs.lanky:A.lankyAccess=_C
		elif C==Kongs.tiny:A.tinyAccess=_C
		else:A.chunkyAccess=_C
...
class TransitionBack:
	...
	def __init__(A,regionId,exitName,spoilerName,reverse=_B):'Initialize with given parameters.';A.regionId=regionId;A.name=exitName;A.spoilerName=spoilerName;A.reverse=reverse
