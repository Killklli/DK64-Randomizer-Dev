...
def RandomizePrices(weight):
	'Generate randomized prices based on given weight (free, low, medium, or high).';D=weight;B=VanillaPrices.copy();A=5.3571;E=A*0.25
	if D=='high':A=7.8571;E=A*0.2
	elif D=='low':A=2.8571;E=A*0.25
	for C in B.keys():
		if C in ProgressiveMoves.keys():
			B[C]=[]
			for F in range(ProgressiveMoves[C]):B[C].append(GenerateRandomPrice(D,A,E))
		else:B[C]=GenerateRandomPrice(D,A,E)
	return B
