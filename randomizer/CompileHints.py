...
def compileHints(spoiler):
	...
	if H.settings.shuffle_items=='moves'and H.move_data is not None:
		...
		for K in T:
			X=random.choice(S[K[A]]);n=K[C];J=random.choice(M[K[B]]);o=random.choice(m[K[D]]);Y=f"{n} can be purchased in {J} from {o}"
