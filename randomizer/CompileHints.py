...
def compileHints(spoiler):
	'Push hints to hint list based on settings.';m='Frantic Factory';l='Jungle Japes';k='Candy';j='Funky';i='Cranky';h='Their first special move';g='Their third special move';O='name_cryptic';M='color';G=spoiler;F='important';E='key';D='shop';C='name';B='level';A='kong'
	...
	if G.settings.shuffle_items=='moves'and G.move_data is not None:
		...
		for J in R:
			V=random.choice(Q[J[A]]);q=J[C];I=random.choice(L[J[B]]);r=p[J[D]];W=f"{q} can be purchased in {I} from {r}."
			if J[F]:updateRandomHint(W)
			else:K.append(W)
	if G.settings.kong_rando:
		s=G.shuffled_kong_placement;t=[{C:l,B:0},{C:'Llama Temple',B:1},{C:'Tiny Temple',B:1},{C:m,B:2}]
		for X in t:u=s[X[C]]['locked'][A];v=X[B];V=random.choice(Q[u]);I=random.choice(L[v]);updateRandomHint(f"{V} can be found in {I}.")
	...
	y=[k,j,i];z=[' Donkey',' Diddy',' Lanky',' Tiny',' Chunky',' Shared'];Y=[A for A in G.woth.keys()if any((B in A for B in y))];A0=random.sample(Y,min(5,len(Y)))
	for Z in A0:
		a=[A for A in z if A in Z]
		if len(a)>0:A1=str(Z).removesuffix(a[0])
		updateRandomHint(f"{A1} is on the Way of the Hoard.")
	b=[l,'Angry Aztec',m,'Gloomy Galleon','Fungi Forest','Crystal Caves','Creepy Castle'];A2=[{A:'Donkey',M:'Yellow'},{A:'Diddy',M:'Red'},{A:'Lanky',M:'Blue'},{A:'Tiny',M:'Purple'},{A:'Chunky',M:'Green'}];K.append(f"You can find bananas in {random.choice(b)}, but also in other levels.");c=random.choice(A2);K.append(f"{c[A]} can find {c[M]} bananas in {random.choice(b)}.")
	for H in range(8):
		N=G.settings.EntryGBs[H];d='Golden Bananas'
		if N==1:d='Golden Banana'
		I=random.choice(L[H]);K.append(f"The barrier to {I} can be cleared by obtaining {N} {d}.")
	for H in range(7):
		N=G.settings.BossBananas[H];e='Small Bananas'
		if N==1:e='Small Banana'
		I=random.choice(L[H]);K.append(f"The barrier to the boss in {I} can be cleared by obtaining {N} {e}.")
	f=35
	if len(K)<35:f=len(K)
	random.shuffle(K)
	for H in range(f):updateRandomHint(K[H])
