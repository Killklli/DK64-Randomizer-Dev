...
def compileHints(spoiler):
	'Push hints to hint list based on settings.';i='Frantic Factory';h='Jungle Japes';g='Their first special move';f='Their third special move';Q=True;P='name_cryptic';N='color';H=spoiler;G=False;F='important';E='key';D='shop';C='name';B='level';A='kong'
	...
	if H.settings.shuffle_items=='moves'and H.move_data is not None:
		...
		for K in T:
			X=random.choice(S[K[A]]);m=K[C];J=random.choice(M[K[B]]);n=l[K[D]];Y=f"{m} can be purchased in {J} from {n}."
			if K[F]:updateRandomHint(Y)
			else:L.append(Y)
	if H.settings.kong_rando:
		o=H.shuffled_kong_placement;p=[{C:h,B:0},{C:'Llama Temple',B:1},{C:'Tiny Temple',B:1},{C:i,B:2}]
		for Z in p:q=o[Z[C]]['locked'][A];r=Z[B];X=random.choice(S[q]);J=random.choice(M[r]);updateRandomHint(f"{X} can be found in {J}.")
	...
	if H.settings.level_randomization!='level_order':
		for I in H.settings.krool_keys_required:s=I-4;J=random.choice(M[s]);updateRandomHint(f"You will need to obtain the key from {J} to fight your greatest foe.")
	for I in range(7):
		t=H.settings.boss_maps[I];J=random.choice(M[I])
		if t==199:updateRandomHint(f"The cardboard boss can be found in {J}.")
	a=[h,'Angry Aztec',i,'Gloomy Galleon','Fungi Forest','Crystal Caves','Creepy Castle'];u=[{A:'Donkey',N:'Yellow'},{A:'Diddy',N:'Red'},{A:'Lanky',N:'Blue'},{A:'Tiny',N:'Purple'},{A:'Chunky',N:'Green'}];L.append(f"You can find bananas in {random.choice(a)}, but also in other levels.");b=random.choice(u);L.append(f"{b[A]} can find {b[N]} bananas in {random.choice(a)}.")
	for I in range(8):
