...
_x='select_keys'
_w='nav-qol-tab'
_v='nav-difficulty-tab'
_u='nav-overworld-tab'
_t='nav-random-tab'
_s='nav-started-tab'
_r='no_logic'
_q='random_medal_requirement'
_p='item_rando_list_selected'
_o='shuffle_items'
_n='enemy_rando'
_m='bonus_barrel_rando'
_l='start_with'
_k='random_prices'
_j='move_rando'
_i='helm_random'
...
_c='level_order'
_b='boss_kong_rando'
_a='boss_location_rando'
_Z='randomize_cb_required_amounts'
_Y='randomize_blocker_required_amounts'
_X='presets'
_W='medal_cb_req'
_V='troff_'
_U='blocker_'
...
def max_randomized_medals(event):
	'Validate medal input on loss of focus.';A=js.document.getElementById(_P)
	if not A.value:A.value=15
	elif 0>int(A.value):A.value=0
	elif int(A.value)>40:A.value=40
@bind(_L,_W)
def max_randomized_medal_cb_req(event):
	'Validate cb medal input on loss of focus.';A=js.document.getElementById(_W)
...
def set_preset_options():
	'Set the Blocker presets on the page.';E='-- Select a Preset --';C=document.getElementById(_X);D=[]
...
def update_boss_required(evt):
	'Disable certain page flags depending on checkboxes.';F=document.getElementById(_Q);A=document.getElementById(_a);B=document.getElementById(_b);C=document.getElementById(_N);E=document.getElementById('move_off');D=document.getElementById('hard_level_progression')
	if F.value==_c:
	...
	elif F.value==_O and C.checked:A.setAttribute(_A,_A);A.checked=_B;B.setAttribute(_A,_A);B.checked=_B;C.removeAttribute(_A);E.removeAttribute(_A);D.setAttribute(_A,_A);D.checked=_C
	else:
...
def disable_boss_rando(evt):
	'Disable Boss Kong and Boss Location Rando if Vanilla levels and Kong Rando.';C=document.getElementById(_Q);A=document.getElementById(_a);B=document.getElementById(_b);D=document.getElementById(_N)
	if D.checked and C.value==_O or C.value==_c:A.setAttribute(_A,_A);A.checked=_B;B.setAttribute(_A,_A);B.checked=_B
...
def disable_move_shuffles(evt):
	'Disable some settings based on the move rando setting.';E='normal';D=js.document.getElementById(_j);C=js.document.getElementById(_k);A=js.document.getElementById('training_barrels');B=js.document.getElementById(_T)
	try:
		if D.value==_l:C.setAttribute(_A,_A);A.value=E;A.setAttribute(_A,_A);B.value=_O;B.setAttribute(_A,_A)
		elif D.value=='off':C.removeAttribute(_A);A.value=E;A.setAttribute(_A,_A);B.value=_O;B.setAttribute(_A,_A)
...
def disable_coupled_camera_shockwave(evt):
	'Change shockwave/camera selection to decoupled if shops are shuffled.';B=_C;D=document.getElementById(_p).options;A=document.getElementById('shockwave_status_shuffled')
	for C in D:
...
def preset_select_changed(event):
	'Trigger a change of the form via the JSON templates.';F=document.getElementById(_X);B=_E
...
def toggle_medals_box(event):
	...
	B=js.document.getElementById(_P)
	if A:B.setAttribute(_A,_A)
	else:B.removeAttribute(_A)
@bind(_F,_T)
def toggle_extreme_prices_option(event):
	'Determine the visibility of the extreme prices option.';C=document.getElementById(_T).value==_l;D=document.getElementById(_r).checked;A=document.getElementById('extreme_price_option')
...
def toggle_patch_ui(event):
	...
	for A in [_s,_t,_u,_v,_w]:document.getElementById(A).setAttribute(_A,_A)
...
def toggle_patch_ui(event):
	...
	for B in [_s,_t,_u,_v,_w]:document.getElementById(B).removeAttribute(_A)
	document.getElementById('override_div').setAttribute(A,A)
@bind(_D,_x)
def toggle_key_settings(event):
	'Disable other keys settings when selecting keys. Toggle Key Selector Modal.';B=_C
	if js.document.getElementById(_x).checked:B=_B
	A=js.document.getElementById('krool_access');C=js.document.getElementById('keys_random');D=js.document.getElementById('starting_keys_list_modal')
	if B:A.setAttribute(_A,_A);A.checked=_C;C.setAttribute(_A,_A);D.removeAttribute(_A)
	else:A.removeAttribute(_A);C.removeAttribute(_A);D.setAttribute(_A,_A)