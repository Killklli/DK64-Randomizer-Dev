...
import randomizer.LogicFiles.DKIsles,randomizer.LogicFiles.JungleJapes,randomizer.LogicFiles.AngryAztec,randomizer.LogicFiles.FranticFactory,randomizer.LogicFiles.GloomyGalleon,randomizer.LogicFiles.FungiForest,randomizer.LogicFiles.CrystalCaves,randomizer.LogicFiles.CreepyCastle
class Settings:
	...
	def resolve_settings(A):
		'Resolve settings which are not directly set through the UI.';V='levels';U='random';T='skip';S='region';L=GetKongs();A.shuffled_location_types=[]
		if A.shuffle_items:
			if not A.item_rando_list_selected:A.shuffled_location_types=[Types.Shop,Types.Banana,Types.Crown,Types.Blueprint,Types.Key,Types.Medal,Types.Coin]
				...
				for W in A.item_rando_list_selected:
					for type in Types:
						if type.name==W.capitalize():A.shuffled_location_types.append(type)
		...
		A.shuffle_prices();A.update_progression_totals();A.krool_donkey=_A;A.krool_diddy=_A;A.krool_lanky=_A;A.krool_tiny=_A;A.krool_chunky=_A;G=L.copy()
		...
		A.helm_order=F
		if A.random_starting_region:
			I=[randomizer.LogicFiles.DKIsles.LogicRegions,randomizer.LogicFiles.JungleJapes.LogicRegions,randomizer.LogicFiles.AngryAztec.LogicRegions,randomizer.LogicFiles.FranticFactory.LogicRegions,randomizer.LogicFiles.GloomyGalleon.LogicRegions,randomizer.LogicFiles.FungiForest.LogicRegions,randomizer.LogicFiles.CrystalCaves.LogicRegions,randomizer.LogicFiles.CreepyCastle.LogicRegions];N=random.choice(I);O=[]
			for J in N:
				I=N[J];X=[A.exitShuffleId for A in I.exits if A.exitShuffleId is not _B and A.exitShuffleId in ShufflableExits and ShufflableExits[A.exitShuffleId].back.reverse is not _B]
				if J in RegionMapList:
					Y=GetMapId(J)
					for Z in X:P=ShufflableExits[Z].back.reverse;a=GetExitId(ShufflableExits[P].back);O.append({S:J,'map':Y,'exit':a,'region_name':I.name,'exit_name':ShufflableExits[P].back.name})
			A.starting_region=random.choice(O)
			for M in range(2):randomizer.LogicFiles.DKIsles.LogicRegions[Regions.GameStart].exits[M+1].dest=A.starting_region[S]
		Q=[Events.JapesKeyTurnedIn,Events.AztecKeyTurnedIn,Events.FactoryKeyTurnedIn,Events.GalleonKeyTurnedIn,Events.ForestKeyTurnedIn,Events.CavesKeyTurnedIn,Events.CastleKeyTurnedIn,Events.HelmKeyTurnedIn];C=Q.copy();K=0
		if A.keys_random:K=randint(0,8)
		if A.select_keys:
			A.krool_keys_required=Q.copy()
		...
		else:K=A.krool_key_count
		if A.krool_access or A.win_condition=='get_key8':A.krool_keys_required.append(Events.HelmKeyTurnedIn);C.remove(Events.HelmKeyTurnedIn);K-=1
		if not A.select_keys:
			random.shuffle(C)
			for M in range(K):A.krool_keys_required.append(C[M])
		...
		if A.bonus_barrel_auto_complete:A.bonus_barrels=T
		elif A.bonus_barrel_rando and not A.minigames_list_selected:A.bonus_barrels=U
		elif A.bonus_barrel_rando and A.minigames_list_selected:A.bonus_barrels='selected'
		if A.helm_setting=='skip_all':A.helm_barrels=T
		elif A.bonus_barrel_rando:A.helm_barrels=U
		if A.level_randomization=='level_order':A.shuffle_loading_zones=V
		...
		if A.kong_rando:
			A.starting_kong_list=random.sample(L,A.starting_kongs_count);A.starting_kong=random.choice(A.starting_kong_list);A.diddy_freeing_kong=Kongs.any;A.lanky_freeing_kong=Kongs.any;A.tiny_freeing_kong=Kongs.any;A.chunky_freeing_kong=Kongs.any
			...
			A.possible_kong_list=L.copy();A.possible_kong_list.remove(0);A.starting_kong_list=random.sample(A.possible_kong_list,A.starting_kongs_count-1);A.starting_kong_list.append(Kongs.donkey);A.starting_kong=Kongs.donkey;A.diddy_freeing_kong=Kongs.donkey;A.lanky_freeing_kong=Kongs.donkey;A.tiny_freeing_kong=Kongs.diddy;A.chunky_freeing_kong=Kongs.lanky;A.kong_locations=[Locations.DiddyKong,Locations.LankyKong,Locations.TinyKong,Locations.ChunkyKong]
		...
		if A.starting_kongs_count<5 and(A.shuffle_loading_zones==V or A.shuffle_loading_zones==_F)and not A.no_logic:A.kongs_for_progression=_C
		...
		if A.win_condition=='all_blueprints':
			for R in ItemList:
				if ItemList[R].type==Types.Blueprint:ItemList[R].playthrough=_C
