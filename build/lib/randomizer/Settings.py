...
class Settings:
	...
	def resolve_settings(A):
		'Resolve settings which are not directly set through the UI.';M='levels';L='random';K='skip';G=GetKongs();A.shuffled_location_types=[]
		if A.shuffle_items:
			if not A.item_rando_list_selected:A.shuffled_location_types=[Types.Shop,Types.Banana,Types.Crown,Types.Blueprint,Types.Key,Types.Medal,Types.Coin]
				...
				for N in A.item_rando_list_selected:
					for type in Types:
						if type.name==N.capitalize():A.shuffled_location_types.append(type)
		...
		A.progressives_locked_in_shops=_A;A.shuffle_prices();A.update_progression_totals();A.krool_donkey=_A;A.krool_diddy=_A;A.krool_lanky=_A;A.krool_tiny=_A;A.krool_chunky=_A;E=G.copy()
		if A.krool_phase_order_rando:random.shuffle(E)
		if A.krool_random:A.krool_phase_count=randint(1,5)
		if isinstance(A.krool_phase_count,str)is _C:A.krool_phase_count=5
		if A.krool_phase_count<5:E=random.sample(E,A.krool_phase_count)
		C=[]
		for B in E:
			...
			if B==Kongs.chunky:A.krool_chunky=_C;C.append(Kongs.chunky)
		A.krool_order=C;A.helm_donkey=_A;A.helm_diddy=_A;A.helm_lanky=_A;A.helm_tiny=_A;A.helm_chunky=_A;F=[Kongs.donkey,Kongs.chunky,Kongs.tiny,Kongs.lanky,Kongs.diddy]
		if A.helm_phase_order_rando:random.shuffle(F)
		if A.helm_random:A.helm_phase_count=randint(1,5)
		if isinstance(A.helm_phase_count,str)is _C:A.helm_phase_count=5
		if A.helm_phase_count<5:F=random.sample(F,A.helm_phase_count)
		D=[]
		for B in F:
			if B==Kongs.donkey:D.append(0);A.helm_donkey=_C
			elif B==Kongs.diddy:A.helm_diddy=_C;D.append(4)
			elif B==Kongs.lanky:A.helm_lanky=_C;D.append(3)
			elif B==Kongs.tiny:A.helm_tiny=_C;D.append(2)
			elif B==Kongs.chunky:A.helm_chunky=_C;D.append(1)
		A.helm_order=D;O=[Events.JapesKeyTurnedIn,Events.AztecKeyTurnedIn,Events.FactoryKeyTurnedIn,Events.GalleonKeyTurnedIn,Events.ForestKeyTurnedIn,Events.CavesKeyTurnedIn,Events.CastleKeyTurnedIn,Events.HelmKeyTurnedIn];H=O.copy()
		if A.keys_random:I=randint(0,8)
		else:I=A.krool_key_count
		if A.krool_access or A.win_condition=='get_key8':A.krool_keys_required.append(Events.HelmKeyTurnedIn);H.remove(Events.HelmKeyTurnedIn);I-=1
		random.shuffle(H)
		for P in range(I):A.krool_keys_required.append(H[P])
		...
		if A.bonus_barrel_auto_complete:A.bonus_barrels=K
		elif A.bonus_barrel_rando and not A.minigames_list_selected:A.bonus_barrels=L
		elif A.bonus_barrel_rando and A.minigames_list_selected:A.bonus_barrels='selected'
		if A.helm_setting=='skip_all':A.helm_barrels=K
		elif A.bonus_barrel_rando:A.helm_barrels=L
		...
		if A.starting_kongs_count<5 and(A.shuffle_loading_zones==M or A.shuffle_loading_zones==_F)and not A.no_logic:A.kongs_for_progression=_C
		...
		if A.win_condition=='all_blueprints':
			for J in ItemList:
				if ItemList[J].type==Types.Blueprint:ItemList[J].playthrough=_C
	...
	def update_valid_locations(A):
		...
		if any(A.shuffled_location_types):
			...
			if Types.Crown in A.shuffled_location_types:A.valid_locations[Types.Crown]=C
