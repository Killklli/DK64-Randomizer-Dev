...
def place_randomized_items(spoiler):
	'Place randomized items into ROM.';Y='shared';X='kong';W='id';R='upscale';O=spoiler;N='flag';M='big';L=None;G='obj'
	if O.settings.shuffle_items:
		H=O.settings.rom_data;ROM().seek(H+52);ROM().write(1);d=O.item_assignment;e=[116,222,224,225,221,223,72,655,316,397,144,648];I={};P=0;S=[]
		for A in d:
			if A.can_have_item:
				if A.is_shop:
					...
					for Z in A.placement_index:
						Q=f+4*Z
						if A.new_item is L:
							ROM().seek(Q);g=int.from_bytes(ROM().readBytes(4),M)
							if g==3758161919 or Z>=120:ROM().seek(Q);ROM().writeMultipleBytes(7<<5,1);ROM().writeMultipleBytes(0,1);ROM().writeMultipleBytes(65535,2)
						elif A.new_flag&32768:
							h=A.new_flag>>12&7;a=A.new_flag>>8&15
							if a==7:b=0
							else:b=(A.new_flag&255)-1
							ROM().seek(Q);ROM().writeMultipleBytes(a<<5|b<<3|h,1);ROM().writeMultipleBytes(A.price,1);ROM().writeMultipleBytes(65535,2)
						else:
							J=5
							if A.new_item==Types.Banana:J=6
							ROM().seek(Q);ROM().writeMultipleBytes(J<<5,1);ROM().writeMultipleBytes(A.price,1);ROM().writeMultipleBytes(A.new_flag,2)
				elif not A.reward_spot:
					for E in A.placement_data:
						if E not in I:I[E]=[]
						if A.new_item is L:I[E].append({W:A.placement_data[E],G:Types.NoItem,X:0,N:0,R:1,Y:False})
						else:i=model_two_scales[A.new_item];j=model_two_scales[A.old_item];k=i/j;I[E].append({W:A.placement_data[E],G:A.new_item,X:A.new_kong,N:A.new_flag,R:k,Y:A.shared})
					if A.location==Locations.NintendoCoin and A.new_item==Types.Banana:ROM().seek(H+272);ROM().write(1)
						...
						ROM().seek(33493504+4*P);ROM().writeMultipleBytes(A.old_flag,2);ROM().writeMultipleBytes(B,1);P+=1
				else:
					...
					if A.old_item==Types.Blueprint:T=A.old_flag-469;ROM().seek(33492992+T);ROM().write(B)
						...
						n=[Types.Banana,Types.Blueprint,Types.Key,Types.Crown,Types.Coin,Types.Medal,Types.Shop,Types.Shop,Types.Shop,Types.TrainingBarrel,Types.Shockwave,L];T=A.old_flag-549;ROM().seek(33493120+T)
						if A.new_item==Types.Shop:
							J=A.new_flag>>8&15;U=6
							if J==4:U=8
							elif J==2 or J==3:U=7
							ROM().write(U)
					...
					elif A.old_item==Types.Banana:ROM().seek(33493504+4*P);ROM().writeMultipleBytes(A.old_flag,2);ROM().writeMultipleBytes(B,1);P+=1
			if not A.is_shop and A.can_have_item:
				V=[A.old_flag]
				if A.new_item is L:V.append(0)
				else:V.append(A.new_flag)
				S.append(V)
		S.append([65535,65535]);ROM().seek(33497088)
		for o in sorted(S,key=lambda x:x[0]):
					...
					if D[W]==r:
						...
						if s in e:
							...
							if D[G]==Types.Blueprint:K=model_two_indexes[Types.Blueprint][D[X]]
								...
								if D[N]==379:K=model_two_indexes[Types.Coin][1]
							elif D[G]==Types.Shop:
								if D[N]&32768==0:C=5
								else:
									C=D[N]>>12&7
							...
							ROM().writeMultipleBytes(K,2);ROM().seek(F+12);t=intf_to_float(int.from_bytes(ROM().readBytes(4),M));u=t*D[R];ROM().seek(F+12);ROM().writeMultipleBytes(int(float_to_hex(u),16),4)
							if D[G]==Types.Blueprint:ROM().seek(F+4);v=intf_to_float(int.from_bytes(ROM().readBytes(4),M));w=v+D[R]*1.25;ROM().seek(F+4);ROM().writeMultipleBytes(int(float_to_hex(w),16),4)