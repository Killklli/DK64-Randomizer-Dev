...
_c='gnawty_barrels'
_b='krool_random'
_a='starting_kongs_count'
_Z='random_music'
_Y='random_colors'
_X='level_order'
_W='shop_location_rando'
_V='kong_rando'
_U='randomize_cb_required_amounts'
_T='randomize_blocker_required_amounts'
_S='focusout'
_R='troff_'
_Q='blocker_'
_P='enable_tag_anywhere'
_O='change'
_N='unlock_all_moves'
...
def set_preset_options():
	'Set the Blocker presets on the page.';E='-- Select a Preset --';C=document.getElementById(_L);D=[]
...
def update_boss_required(evt):
	'Disable certain page flags depending on checkboxes.';E=document.getElementById(_M);B=document.getElementById('boss_location_rando');C=document.getElementById('boss_kong_rando');D=document.getElementById(_V);A=document.getElementById(_W);F=document.getElementById(_N)
	if E.value==_X:B.setAttribute(_A,_A);B.checked=_B;C.setAttribute(_A,_A);C.checked=_B;D.setAttribute(_A,_A);D.checked=_B;A.setAttribute(_A,_A);A.checked=_B
	else:
...
def disable_colors(evt):
	...
	if js.document.getElementById(_Y).checked:A=_B
...
def disable_tag_spawn(evt):
	...
	if js.document.getElementById(_P).checked is _C:A=_B
...
def enable_tag_anywhere(evt):
	...
	if js.document.getElementById(_I).checked:js.document.getElementById(_P).checked=_B
@bind(_E,_Z)
def disable_music(evt):
	'Disable music options when Randomize All is selected.';A=_C
	if js.document.getElementById(_Z).checked:A=_B
...
def enable_kong_rando(evt):
	'Enable Kong Rando if less than 5 starting kongs.';print('Lanky');A=js.document.getElementById(_V)
	if js.document.getElementById(_a).value=='5':A.checked=_C;A.setAttribute(_A,_A)
...
def disable_shuffle_shop(evt):
	'Disable Shuffle Shop Move Location when All Moves are Unlocked.';C=_C;A=js.document.getElementById(_W);B=js.document.getElementById('random_prices');D=js.document.getElementById(_N)
	if D.checked:C=_B
	try:
		if C:A.setAttribute(_A,_A);A.checked=_C;B.setAttribute(_A,_A)
		elif js.document.getElementById(_M).value!=_X:A.removeAttribute(_A);B.removeAttribute(_A)
...
def disable_barrel_rando(evt):
	...
	if js.document.getElementById(_c).checked:B=_B
	try:
...
def preset_select_changed(event):
	'Trigger a change of the form via the JSON templates.';D=document.getElementById(_L);B=_D
