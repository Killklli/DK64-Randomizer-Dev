...
def max_randomized_fairies(event):
	'Validate fairy input on loss of focus.';A=js.document.getElementById(_X)
	if not A.value:A.value=20
	elif 1>int(A.value):A.value=1
	elif int(A.value)>20:A.value=20
...
def update_boss_required(evt):
	'Disable certain page flags depending on checkboxes.';F=document.getElementById(_Q);A=document.getElementById(_b);B=document.getElementById(_c);C=document.getElementById(_N);E=document.getElementById('move_off');D=document.getElementById('hard_level_progression')
	if F.value==_d:
...
def disable_boss_rando(evt):
	'Disable Boss Kong and Boss Location Rando if Vanilla levels and Kong Rando.';C=document.getElementById(_Q);A=document.getElementById(_b);B=document.getElementById(_c);D=document.getElementById(_N)
	if D.checked and C.value==_O or C.value==_d:A.setAttribute(_A,_A);A.checked=_B;B.setAttribute(_A,_A);B.checked=_B
...
def disable_move_shuffles(evt):
	...
	try:
		if D.value==_m:C.setAttribute(_A,_A);A.value=E;A.setAttribute(_A,_A);B.value=_O;B.setAttribute(_A,_A)
...
def disable_coupled_camera_shockwave(evt):
	'Change shockwave/camera selection to decoupled if shops are shuffled.';B=_C;D=document.getElementById(_q).options;A=document.getElementById('shockwave_status_shuffled')
	for C in D:
...
def toggle_key_settings(event):
	...
	if js.document.getElementById(_y).checked:B=_B
